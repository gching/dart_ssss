 ...
 * specifically with Shamir's Secret Sharing Scheme.
 */
class SecretScheme {
  ...
  /**
   * Getter for the number of parts for the current instance.
   */
  int get numOfParts => _numOfParts;

  /**
   * Getter for the threshold for the current instance.
   */
  ...
  List<int> combineShares(Map<int, List<int>> shares) {
    ...
    for (List<int> yCoords in shares.values) {
      ...
      if (yCoords.length != lengthOfYCoords){
        throw new ArgumentError('Shares y coordinates need to have the same '
            + 'length');
      }
