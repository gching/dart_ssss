/**
 *  gf_256.dart
 */

import 'utils/byte_helper.dart';

/**
 * Implementation of operations on finite field GF(2^8) or GF(256) to be
 * utilized with the SSSS. To leare more about GF(256) or why it was selected,
 * please refer to:
 *
 * https://research.swtch.com/field.
 *
 * Important thing to note is that because we have selected a finite field
 * GF(256), it means that we are limited to number of shares cannot be greater
 * then 255 (where one coordinate is saved to decipher the secret).
 */
class GF256 {

  static final List<int> LOG3 = [
    
  ];

  static final List<int> EXP3 = [
    
  ];

  /**
   * Adds the left operand and right operand in GF(256) which is just the
   * bitwise XOR.
   */
  static int add(int leftOp, int rightOp){
    throwIfNotUnsignedByte(leftOp);
    throwIfNotUnsignedByte(rightOp);

    // Perform bitwise XOR.
    return leftOp ^ rightOp;
  }

  /**
   * Subtracts the left operand and right operand in GF(256) which is add.
   */
  static int sub(int leftOp, int rightOp) => add(leftOp, rightOp);

  /**
   * Multplies the left operand and right operand in GF(256) which utilizes
   * exponential and log lookups given a generator of 0x03 (x + 1 polynomial).
   *
   * To learn more about this method:
   *
   * https://en.wikipedia.org/wiki/Finite_field_arithmetic#Implementation_tricks
   *
   */
  static int multiply(int leftOp, int rightOp) {
    throwIfNotUnsignedByte(leftOp);
    throwIfNotUnsignedByte(rightOp);

    // In the case that one of them is zero.
    if (leftOp == 0 || rightOp == 0){
      return 0;
    }
    // Find the log3 of both the left operand and right operand
    // add them together and 3 to the power of the the addition results.
    //
    // results = 3^(log3(leftOp) + log3(rightOp))

    return EXP3[LOG3[leftOp] + LOG3[rightOp]];
  }

  /**
   * Does division given the dividend and divsor (dividend / divisor).
   * which utilizes lookups given a generator of 0x03 (x+1 polynomial).
   */
  static int divide(int dividend, int divisor){
    throwIfNotUnsignedByte(dividend);
    throwIfNotUnsignedByte(divisor);

    // 0 divided by anything is 0.
    if (dividend == 0){
      return 0;
    }

    // We calculate the multiplicative inverse which is defined by
    // inv = 3^(255 - log3(divisor))
    // Then we multiply the inv with the dividend which is the division results
    // of GF(256)
    return multiply(dividend, EXP3[255 - LOG3[divisor]]);

  }

  /**
   * Throws a byte exception error if the value is not a byte.
   */
  static void throwIfNotUnsignedByte(int possibleVal) {
    if (ByteHelper.isNotByte(possibleVal)) {
      throw new ArgumentError.value(possibleVal);
    }
  }

}

